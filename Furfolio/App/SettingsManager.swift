...
final class SettingsManager: ObservableObject {
    ...
    private init(defaults: UserDefaults = .standard) {
        ...
        $fontSizeScale
            ...
            .sink { [defaults] in defaults.set($0, forKey: Keys.fontSizeScale) }
            .store(in: &cancellables)
        $defaultReminderOffset
            .debounce(for: .milliseconds(150), scheduler: RunLoop.main)
            .sink { [defaults] in defaults.set($0, forKey: Keys.defaultReminderOffset) }
            .store(in: &cancellables)
        $loyaltyThreshold
            .debounce(for: .milliseconds(150), scheduler: RunLoop.main)
            .sink { [defaults] in defaults.set($0, forKey: Keys.loyaltyThreshold) }
            .store(in: &cancellables)
        $rewardThresholds
